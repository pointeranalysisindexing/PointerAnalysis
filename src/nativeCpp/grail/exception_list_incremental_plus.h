...
class ExceptionListIncrementalPlus : public ExceptionListIncrementalBase{
	...
	ExceptionListIncrementalPlus(Graph& org, int dim, bool lf){
		...
		lists = new unordered_set<int>[size];
		directs = new unordered_set<int>[size];
		indirects = new unordered_set<int>[size];
		...
		for(i=0;i<size;i++){
			set_union(directs[i].begin() , directs[i].end(), indirects[i].begin(), indirects[i].end(),
				std::insert_iterator<unordered_set<int> >(lists[i],lists[i].end()));
...
//		std::pair<AdjItr, AdjItr> out;
		...
		Vertex child;
		unordered_set<int>::iterator setItr;
		set<int> merged;
		bool flag;
		int numberOfCopy;
		for(i=0;i<vlist.size();i++){
